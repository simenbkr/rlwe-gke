...
void create_player(player_t *player, int n, int id) {
    ...
    crypto_sign_public_key(player->verification_key, player->secret_key);
...
void authenticated_key_exchange(player_t *init, player_t *resp) {
    /**
     * Initiator signs his public key.
     */
    uint8_t message[32];
    uint8_t sign[64];
    bzero(message, 32);
    memcpy(message, init->public_key, 32);
    crypto_sign(sign, init->secret_key, init->verification_key, message, 32);

    /**
     * Responder verifies the signature.
     */

    if(crypto_check(sign, init->verification_key, message, 32)) {
        printf("Could not verify the signature!\n");
    }

    /**
     * Key exchange as normal.
     */
    uint8_t session_key[32];
    crypto_key_exchange(session_key, init->secret_key, resp->public_key);
    init->session_keys[init->i++] = session_key;
}
...
int test_n_players(int n, uint64_t *clock_cycles, int cycle_index) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        for (int j = 0; j < n; ++j) {
            ...
            authenticated_key_exchange(init, resp);
        }
    }
