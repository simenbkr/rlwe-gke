...
void poly_subtract(poly *a, poly *b, poly *result) {
    for (int i = 0; i < 256; i++) {
        result->coefficients[i      ] = barrett_64(a->coefficients[i      ] - b->coefficients[i      ] + GKE_Q * 3);
        result->coefficients[256 + i] = barrett_64(a->coefficients[256 + i] - b->coefficients[256 + i] + GKE_Q * 3);
        result->coefficients[512 + i] = barrett_64(a->coefficients[512 + i] - b->coefficients[512 + i] + GKE_Q * 3);
        result->coefficients[768 + i] = barrett_64(a->coefficients[768 + i] - b->coefficients[768 + i] + GKE_Q * 3);
    }
}

void pointwise_mult_mod(poly *a, poly *b, poly *result) {
    for (int i = 0; i < GKE_N; i++) {
        result->coefficients[i] = (a->coefficients[i] * b->coefficients[i]) % GKE_Q;
    }
}
...
void poly_to_uint8(const poly *a, uint8_t *out) {
    for (int i = 0; i < GKE_N; ++i) {
        out[8 * i + 0] = (uint8_t)(a->coefficients[i] >> 56) & 0xff;
        out[8 * i + 1] = (uint8_t)(a->coefficients[i] >> 48) & 0xff;
        out[8 * i + 2] = (uint8_t)(a->coefficients[i] >> 40) & 0xff;
        out[8 * i + 3] = (uint8_t)(a->coefficients[i] >> 32) & 0xff;
        out[8 * i + 4] = (uint8_t)(a->coefficients[i] >> 24) & 0xff;
        out[8 * i + 5] = (uint8_t)(a->coefficients[i] >> 16) & 0xff;
        out[8 * i + 6] = (uint8_t)(a->coefficients[i] >> 8) & 0xff;
        out[8 * i + 7] = (uint8_t)(a->coefficients[i] >> 0) & 0xff;
