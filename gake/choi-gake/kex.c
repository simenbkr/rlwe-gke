...
void rec_msg_ct(const poly *b, poly *rec) {
    ...
    uint64_t cond0 = GKE_Q / (uint64_t) 2;
    uint64_t cond1 = GKE_Q;
    uint64_t cond2 = (uint64_t) 3 * GKE_Q / (uint64_t) 2;
    uint64_t cond3 = (uint64_t) 2 * GKE_Q;
...
void calculate_b_from_playerlist(player **playerlist, int player_count, player *self) {
    ...
    for (int j = 0; j < GKE_N; ++j) {
        self->b.coefficients[j] = self->b.coefficients[j] % GKE_Q;
    }
}