...
static const uint64_t msg_cond0 = APON_Q / (uint64_t) 2;
static const uint64_t msg_cond1 = APON_Q;
static const uint64_t msg_cond2 = (uint64_t) 3 * APON_Q / (uint64_t) 2;
static const uint64_t msg_cond3 = (uint64_t) 2 * APON_Q;
...
void rec_msg_ct(const poly *b, poly *rec) {
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        for (int j = 0; j < 64; ++j) {
            ...
            rec->coefficients[i * 64 + j] = (ct_ge_u64(t, msg_cond0) & ct_le_u64(t, msg_cond1)) | (ct_ge_u64(t, msg_cond2) & ct_le_u64(t, msg_cond3));
            r >>= 1;
        }
    }
}
...
static const uint64_t new_q = APON_Q * (uint64_t) 2;
static const uint64_t one_eight = new_q / (uint64_t) 8;
static const uint64_t key_cond1 = (uint64_t) 3 * one_eight + (uint64_t) 1;
static const uint64_t key_cond2 = (uint64_t) 7 * one_eight + (uint64_t) 2;
static const uint64_t key_cond3 = (uint64_t) one_eight;
static const uint64_t key_cond4 = (uint64_t) 5 * one_eight + (uint64_t) 1;

//Constant time code adapted from BCNS RLWE.
void rec_key_ct(const poly *rec, const poly *b, poly *key) {

    zero_out(key);
    for (int i = 0; i < APON_N; ++i) {
        uint64_t t = b->coefficients[i] << (uint64_t) 1;
        key->coefficients[i] = (ct_eq_u64(rec->coefficients[i], 0) & ct_ge_u64(t, key_cond1) & ct_le_u64(t, key_cond2)) |
                (ct_eq_u64(rec->coefficients[i], 1) & ct_ge_u64(t, key_cond3) & ct_le_u64(t, key_cond4));
    }
}
...
void calculate_b_from_playerlist(player **playerlist, int player_count, player *self) {
    ...
    for (int j = 0; j < APON_N; ++j) {
        self->b.coefficients[j] = barrett_64(self->b.coefficients[j]);
    }
}