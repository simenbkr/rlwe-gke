...
void gen_poly(poly *a, uint64_t sigma) {
    ...
    if(sigma > MAX_SAMPLER) {
        ...
        while (sum != sigma_squared) {
            ...
            for (int i = 0; i < APON_N; ++i) {
                uint64_t t = D->call(D);
                a->coefficients[i] = modq_64(modq_64(t) + a->coefficients[i]);
            }
        ...
        dgs_disc_gauss_dp_t *D = dgs_disc_gauss_dp_init(sigma, 0, MAX_WIDTH, DGS_DISC_GAUSS_DEFAULT);

        // The thingy is seeded at compile-time. For testing purposes, we introduce a little chaos >:)
        for(int k = 0; k < (random & 0xffff); k++) {
            D->call(D);
        }
...
void poly_add(const poly *a, const poly *b, poly *result) {
    for (int i = 0; i < APON_N; i++) {
        ...
        result->coefficients[i] = modq_64(result->coefficients[i]);
        result->coefficients[i] = modq_64(a->coefficients[i] + b->coefficients[i]);
        //result->coefficients[256 + i] = (a->coefficients[256 + i] + b->coefficients[256 + i]) % APON_Q;
        //result->coefficients[512 + i] = (a->coefficients[512 + i] + b->coefficients[512 + i]) % APON_Q;
        //result->coefficients[768 + i] = (a->coefficients[768 + i] + b->coefficients[768 + i]) % APON_Q;
    }
}
...
uint64_t mulmod(uint64_t a, uint64_t b) {
    ...
    uint64_t a_high = a & 0xFFFFFFFF00000000;
    uint64_t b_low  = b & 0xFFFFFFFF;
    uint64_t b_high = b & 0xFFFFFFFF00000000;

    uint64_t m1 = a_high * b_low;
    uint64_t m2 = a_low * b_high;

    uint64_t m1_low = m1 & 0xFFFFFFFF;
    uint64_t m1_high = m1 >> 32;

    uint64_t m2_low = m2 & 0xFFFFFFFF;
    uint64_t m2_high = m2 >> 32;

    result = a_high * b_high;
    uint64_t tmp = result + ((m1_low << 32) | m1_high);
    if(tmp < result) {
        result = tmp + 1;
    } else {
        result = tmp;
    ...
    while (b > 0) {
        if (b & 1) {
            r += a;
            if (r >= APON_Q) r -= APON_Q;
        }
        b >>= 1;
        if (b) {
            a += a;
            if (a >= APON_Q) a -= APON_Q;
        }
    }
    return r;*/
}
...
void copy_poly(const poly *p, poly *new) {
    zero_out(new);
    for (int i = 0; i < APON_N; ++i) {
...
void poly_to_uint8(const poly *a, uint8_t *out) {
    for (int i = 0; i < APON_N; ++i) {
        out[5 * i + 0] = (uint8_t)((a->coefficients[i] >> 32) & 0xff);
        out[5 * i + 1] = (uint8_t)((a->coefficients[i] >> 24) & 0xff);
        out[5 * i + 2] = (uint8_t)((a->coefficients[i] >> 16) & 0xff);
        out[5 * i + 3] = (uint8_t)((a->coefficients[i] >>  8) & 0xff);
        out[5 * i + 4] = (uint8_t)((a->coefficients[i] >>  0) & 0xff);
...
void uint8_to_poly(const uint8_t *in, poly *a) {
    for (int i = 0; i < APON_N; ++i) {
        a->coefficients[i] = ((uint64_t)in[5 * i + 0] << 32)
                           | ((uint64_t)in[5 * i + 1] << 24)
                           | ((uint64_t)in[5 * i + 2] << 16)
                           | ((uint64_t)in[5 * i + 3] <<  8)
                           | ((uint64_t)in[5 * i + 4] <<  0);
