...
void create_rec_message(const player *p, const poly *rec, rec_message_t *msg) {
    ...
    OQS_SIG_falcon_512_sign(msg->sign, &sign_len, raw, 128 + 2 + 4, p->falcon_priv);
    msg->sign_len = sign_len;
}
...
int verify_rec_message(const player *sender, const rec_message_t *msg) {
    size_t sig_len = OQS_SIG_falcon_512_length_signature;

    uint8_t raw[134];
    create_raw_from_rec_msg(msg, raw, sender->nonce);
    OQS_STATUS status = OQS_SIG_falcon_512_verify(raw, 134, msg->sign, sig_len, sender->falcon_pub);
...
void rec_message_to_uint8(const rec_message_t *msg, uint8_t *buffer) {
    ...
    buffer[4] = (msg->sign_len >> 8) & 0xff;
    buffer[5] = (msg->sign_len >> 0) & 0xff;

    memcpy(buffer + 6, msg->message, 128);
    memcpy(buffer + 6 + 128, msg->sign, SIGN_LENGTH);
...
void uint8_to_rec_message(const uint8_t *raw, rec_message_t *msg) {
    ...
    memcpy(msg->message, raw + 6, 128);
    memcpy(msg->sign, raw + 128 + 6, SIGN_LENGTH);
}
