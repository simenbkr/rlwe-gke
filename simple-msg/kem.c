...
void key_poly_to_uint8(const poly *key, uint8_t *secret) {
    for (int i = 0; i < 128; ++i) {
        secret[i] = (uint8_t)((key->coefficients[8 * i + 0] << 7)
                          | (key->coefficients[8 * i + 1] << 6)
                          | (key->coefficients[8 * i + 2] << 5)
                          | (key->coefficients[8 * i + 3] << 4)
                          | (key->coefficients[8 * i + 4] << 3)
                          | (key->coefficients[8 * i + 5] << 2)
                          | (key->coefficients[8 * i + 6] << 1)
                          | (key->coefficients[8 * i + 7] << 0));
    }
}